...
### 這個版本 是要直接排列 [1~10] 的陣列 依序找答案((就不是使用找鄰近解))
...
resultArrayList = getAllPermutations(array)
print(resultArrayList)
print(len(resultArrayList))
# print(resultArray)
end = time.process_time()
print(f"找到所有[1~10]排列組合的執行時間: ", (end - start))

start2 = time.process_time()

########## STEP 00 __ 設定workMatrix ######################

# WorkMatrix = np.array([[67, 34, 32, 83],
#                        [12, 74, 79, 51],
#                        [32, 17, 87, 10],
#                        [35, 13, 70, 37],
#                        ])
...
## 創建 退火演算法的 method
def SimulatedAnnealing(WorkMatrix, temperature, resultArrayList):  # 要把 workMatrix & 溫度丟進去
    ...
    #### 初代解 ####
    testArray = SolutionArray(resultArrayList[iterationNum])
    print(
    ...
    while iterationNum < resultArrayListNum - 1:  ##在溫度沒有低到"低溫標準"，都繼續執行計算
        iterationNum += 1
        tmpTestArray = SolutionArray(resultArrayList[iterationNum])  ##創建一個tmpTestArray ((從獲得鄰近解method創建
        ### 第一個情況，新的解比舊的解好 ###
...
print(
    f"final_出現在第 {gBestChangeIndexList[-1]} 代, final_gBest = {gBestList[-1].array}, final_gBest_fitness= {gBestListFitness[-1]}")

end2 = time.process_time()

print(f"找到所有[1~10]排列組合的執行時間: ", (end - start))
print(f"找到所有解的執行時間: ", (end2 - start2))
